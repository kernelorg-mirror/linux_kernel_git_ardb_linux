...
union efi_rts_args {
	...
	struct {
		efi_status_t (__efiapi	*acpi_prm_handler)(efi_physical_addr_t,
							   void *);
 ...
 *
 * Accesses to EFI runtime services are serialized by a binary semaphore
 * (efi_runtime_lock) and caller waits until the work is finished, hence _only_
 * one call is queued at a time and the caller thread waits for completion.
 */
...
static void efi_call_rts(struct work_struct *work)
{
	...
	case EFI_ACPI_PRM_HANDLER:
		...
		status = efi_call_virt_pointer(&args->ACPI_PRM_HANDLER,
					       acpi_prm_handler,
...
efi_status_t
efi_call_acpi_prm_handler(efi_status_t (__efiapi *handler_addr)(efi_physical_addr_t,
							        void *),
			  efi_physical_addr_t param_buffer_addr,
			  void *context)
{
