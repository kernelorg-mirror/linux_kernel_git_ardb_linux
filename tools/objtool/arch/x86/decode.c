...
const char *arch_nop_call_insn(int len)
{
	...
	if (len == 6)
		 ...
		 * relax this into 'call __fentry__; nop'. The ftrace code
		 * would be perfectly happy with that, but given that objtool
		 * runs before the linker, it needs to be able to deal with
		 * this as well.  Ftrace will always patch in a direct call,
		 * and so a 5-byte NOP is needed here, plus a 1-byte NOP to
